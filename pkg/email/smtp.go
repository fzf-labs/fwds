	...
	go func() {
		defer func() {
			if err := recover(); err != nil {
				c.chOpen = false
				log.SugaredLogger.Error("send email queue err: %+v, retry after 10 second", err.(error))
		...
		for {
			...
			case m, ok := <-c.ch:
				if !ok {
					log.SugaredLogger.Info("mail queue is close")
					c.chOpen = false
					return
				}
				if !open {
				...
				if err := mail.Send(s, m); err != nil {
					log.SugaredLogger.Warnf("email send failed, %v", err)
				} else {
					log.SugaredLogger.Info("email has send")
				}
			case <-time.After(time.Duration(c.Config.Keepalive) * time.Second):
				if open {
					if err := s.Close(); err != nil {
						log.SugaredLogger.Warnf("can not close smtp conn, %v", err)
					}
				}
				open = false
			}
