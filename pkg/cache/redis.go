...
func (c *redisCache) MultiSet(ctx context.Context, valueMap map[string]interface{}, expiration time.Duration) error {
	...
	for key, value := range valueMap {
		...
		if err != nil {
			log.SugaredLogger.Warnf("marshal data err: %+v, value is %+v", err, value)
			continue
		}
		cacheKey, err := BuildCacheKey(c.KeyPrefix, key)
		if err != nil {
			log.SugaredLogger.Warnf("build cache key err: %+v, key is %+v", err, key)
			continue
		}
		if expiration == 0 {
			expiration = DefaultExpireTime
	...
	for i := 0; i < len(paris); i = i + 2 {
		...
		default:
			log.SugaredLogger.Warnf("redis expire is unsupported key type: %+v", reflect.TypeOf(paris[i]))
		}
	}
	return nil
}
...
func (c *redisCache) MultiGet(ctx context.Context, keys []string, value interface{}) error {
	...
	for i, value := range values {
		...
		if err != nil {
			log.SugaredLogger.Warnf("unmarshal data error: %+v, key=%s, cacheKey=%s type=%v", err,
