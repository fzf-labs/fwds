...
var Time = newUtilTime()

type utilTime struct {
	carbon.Carbon
}

func newUtilTime() *utilTime {
	return &utilTime{carbon.NewCarbon().SetTimezone(carbon.PRC)}
}

func (t *utilTime) NowUnix() int64 {
	return t.Carbon.Now().Timestamp()
}

// NowString 转换为当前时间 2021-06-29 23:53:32
func (t *utilTime) NowString() string {
	return t.Carbon.Now().ToDateTimeString()
}

// NowMillisecondString 转换为当前时间 2021-06-29 23:53:32.010
func (t *utilTime) NowMillisecondString() string {
	now := t.Carbon.Now()
...
func (t *utilTime) NowMicrosecondString() string {
	now := t.Carbon.Now()
...
func (t *utilTime) TimeToShortString(ts time.Time) string {
	return time.Unix(ts.Unix(), 00).Format("2006.01.02")
}

// GetShowTime 格式化人类友好时间
func (t *utilTime) GetShowTime(ts time.Time) string {
	...
	if duration < 60 {
		...
		timeStr = t.TimeToShortString(ts) + "前更新"
	}
	return timeStr
}
...
func (t *utilTime) Date(format string, ts ...time.Time) string {
	patterns := []string{
		// 年
		"Y", "2006", // 4 位数字完整表示的年份
		"y", "06", // 2 位数字表示的年份
...
func (t *utilTime) StrToTime(value string) (tt time.Time, err error) {
	if value == "" {
...
func (t *utilTime) SubDays(t1, t2 time.Time) (day int) {
	swap := false
	if t1.Unix() < t2.Unix() {
		t_ := t1
		t1 = t2
...
func (t *utilTime) StrToLocalTime(value string) (time.Time, error) {
	if value == "" {
	...
	return t.StrToTime(value)
}

// DateFormat 格式化time.Time
// DateFormat("YYYY-MM-DD HH:mm:ss", time.Now())
func (t *utilTime) DateFormat(format string, tt time.Time) string {
...
func (t *utilTime) GetDaysAgoZeroTime(day int) time.Time {
...
func (t *utilTime) TimeToHuman(ts int) string {
	var res = ""
	if ts == 0 {
		return res
	}
