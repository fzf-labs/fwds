...
func withoutBody(method, url string, form httpURL.Values, options ...Option) (body []byte, err error) {
	...
	defer func() {
		if opt.telescope != nil {
			...
			opt.telescope.AppendDialog(opt.dialog)
		}
	...
	if opt.telescope != nil {
		opt.header[telescope.Header] = []string{opt.telescope.ID()}
	}
	...
	if opt.dialog != nil {
		...
		opt.dialog.Request = &telescope.Request{
			TTL:        ttl.String(),
			Method:     method,
