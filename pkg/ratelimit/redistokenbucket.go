...
func (lim *RedisTokenBucket) reserveN(now time.Time, n int) bool {
	...
	if err == redis.Nil {
		...
		log.SugaredLogger.Errorf("fail to use rate limiter: %s, use in-process limiter for rescue", err)
		lim.startMonitor()
		return lim.rescueLimiter.AllowN(now, n)
	}
	...
	code, ok := resp.(int64)
	if !ok {
		log.SugaredLogger.Errorf("fail to eval redis script: %v, use in-process limiter for rescue", resp)
		lim.startMonitor()
		return lim.rescueLimiter.AllowN(now, n)
	}
