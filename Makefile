...
PROJECT_NAME := "fwds"
versionDir = "$(PROJECT_NAME)/pkg/version"
gitTag = $(shell if [ "`git describe --tags --abbrev=0 2>/dev/null`" != "" ];then git describe --tags --abbrev=0; else git log --pretty=format:'%h' -n 1; fi)
buildTime = $(shell TZ=Asia/Shanghai date +%Y%m%d%H%M%S)
gitCommit = $(shell git git rev-parse HEAD)
gitBranch = $(shell git branch 2>/dev/null | grep "^\*" | sed -e "s/^\*\ //")

ldflags="-w -X ${versionDir}.gitTag=${gitTag} -X ${versionDir}.buildTime=${buildDate} -X ${versionDir}.gitCommit=${gitCommit} -X ${versionDir}.gitBranch=${gitBranch}"
