...
var gormName = schema.NamingStrategy{
	SingularTable: true,
}
...
func (g *Generator) ParserAST(p *Parser, structs []string) (ret *Generator) {
	for _, v := range structs {
		...
		g.buf[gormName.TableName(v)] = new(bytes.Buffer)
	}
...
func (g *Generator) checkConfig() (err error) {
	if len(g.config.PkgName) == 0 {
		err = errors.New("package name not set")
		return
	}
...
func (g *Generator) Generate() *Generator {
	...
	for _, v := range g.structConfigs {
		if _, ok := g.buf[gormName.TableName(v.StructName)]; !ok {
			continue
		}
		if err := outputTemplate.Execute(g.buf[gormName.TableName(v.StructName)], v); err != nil {
			panic(err)
		}
	}
...
func (g *Generator) Flush() error {
	for k := range g.buf {
		...
		fmt.Println("  └── file : ", fmt.Sprintf("%s_model/%s.go", strings.ToLower(k), strings.ToLower(k)))
	}
	return nil
}
