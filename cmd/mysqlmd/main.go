...
func init() {
	...
	pass := flag.String("pass", "fzf910423", "请输入 db 密码\n")
	name := flag.String("name", "fuzhifei", "请输入 db 名称\n")
...
func main() {
	...
	if err != nil {
		fmt.Println("query tables of database err", err)
	...
	for _, table := range tables {
		...
		fmt.Println("  └── file : ", table.Name+"/table.md")
		...
		modelContent += fmt.Sprintf("// TableName: %s \n", table.Name)
